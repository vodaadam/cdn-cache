...
Vytvoril jsem funkci ngx_x_cache_key_header_filter, která si vezme už hotový klíč z requestu (je volaná v post configuration, takže klíč už existuje) udělá pushback na headers_out a pushback ji vrátí prázdný ngx_table_elt_t do kterého vyplní už klíč a předá informaci že to je hash. 
...
Jak to je implementováno v nginx?
  - ngx_hash_combined_t si v sobě drží hash tabulku která v sobě má všechny záznamy, zároveň si v sobě drží dvě hash trie - jedna pro suffixi, druhá pro prefixi.
